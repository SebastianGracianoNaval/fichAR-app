...
**fichAR Context:** fichAR uses Flutter (not native Kotlin). Apply these patterns via Flutter Material widgets (Material 3 when API 31+). Minimum: Android 5 (API 21). See fichar-android, fichar-low-end for API 21-23 constraints. Consult `definiciones/FRONTEND.md` for Android-specific UX. that integrate seamlessly with the Android ecosystem.
