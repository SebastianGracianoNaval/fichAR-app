...
## fichAR Context (Adaptation)

**Compatibility:** fichAR targets Android 5+, iOS 12+, Web, Desktop. Always consider low-end devices.

- **Low-end mode:** Detect API < 24 or memory < 2GB; disable or simplify heavy animations (hero, staggered, physics). Use `MediaQuery.disableAnimations` when user has reduced motion.
- **Graceful degradation:** Prefer implicit animations (AnimatedContainer, AnimatedOpacity) on low-end; reserve explicit/Hero for high-end.
- **Duolingo-style feedback:** Scale-down on press, bounce on success; keep duration 200-300ms max.
- **See:** `definiciones/FRONTEND.md`, `documentation/tecnica/ux-feedback-guide.md`, fichar-low-end skill.
