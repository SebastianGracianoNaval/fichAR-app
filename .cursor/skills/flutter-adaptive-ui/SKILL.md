...
Create Flutter applications that adapt gracefully to any screen size, platform, or input device.

**fichAR Context:** fichAR runs on Android 5+ (API 21), iOS 12+, Web (Flutter Web), Windows 10+, macOS 10.14+, Linux. Use LayoutBuilder and MediaQuery breakpoints. Consult `definiciones/FRONTEND.md` for low-end vs high-end detection. Key breakpoints: Mobile < 600px, Tablet 600-840px, Desktop >= 840px. This skill provides comprehensive guidance for building responsive layouts that scale from mobile phones to large desktop displays while maintaining excellent user experience across touch, mouse, and keyboard interactions.
