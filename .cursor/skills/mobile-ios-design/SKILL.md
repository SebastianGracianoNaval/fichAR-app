...
Master iOS Human Interface Guidelines (HIG) and SwiftUI patterns to build polished, native iOS applications.

**fichAR Context:** fichAR uses Flutter (not native Swift). Apply via Flutter Cupertino widgets. Minimum: iOS 12+. See fichar-ios, fichar-low-end. Consult `definiciones/FRONTEND.md` for iOS-specific UX (SF Symbols, Large Title, Safe Area, haptics). that feel at home on Apple platforms.
