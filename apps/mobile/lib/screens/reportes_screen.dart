...
class _ReportesScreenState extends State<ReportesScreen> {
  ...
  Future<void> _exportar() async {
    ...
    if (_desde!.isAfter(_hasta!)) {
      setState(() => _error = 'La fecha desde debe ser anterior a la fecha hasta');
      return;
    }
    setState(() {
      _loading = true;
    ...
    final fi = '${_desde!.year}-${_desde!.month.toString().padLeft(2, '0')}-${_desde!.day.toString().padLeft(2, '0')}';
    final ff = '${_hasta!.year}-${_hasta!.month.toString().padLeft(2, '0')}-${_hasta!.day.toString().padLeft(2, '0')}';
    ...
    final filename = 'fichar-reporte-${DateTime.now().millisecondsSinceEpoch}.$_formato';
    if (kIsWeb) {
      messenger.showSnackBar(
        SnackBar(content: Text('Exportado: $filename (${result.bytes.length} bytes)')),
      );
      return;
    }
    try {
      ...
      if (!mounted) return;
      messenger.showSnackBar(const SnackBar(content: Text('Reporte exportado')));
    } catch (e) {
      if (!mounted) return;
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: Text(_desde != null ? '${_desde!.year}-${_desde!.month.toString().padLeft(2, '0')}-${_desde!.day.toString().padLeft(2, '0')}' : 'Desde'),
                  ),
                ),
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    icon: const Icon(Icons.calendar_today, size: 18),
                    label: Text(_hasta != null ? '${_hasta!.year}-${_hasta!.month.toString().padLeft(2, '0')}-${_hasta!.day.toString().padLeft(2, '0')}' : 'Hasta'),
                  ),
                ),
              ],
            ),
